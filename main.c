...
store_incremented_codeblock (void)
{
	if (code_block < 0xFFFF)
	{
		...
		//sleep_jiffies (JIFFIES_PER_MS (10));
		eeprom_write_byte(0, (u_int8_t) (code_block));
		delay(10);
		eeprom_write_byte(1, (u_int8_t) (code_block >> 8));
		delay(10);
	}
}
...
setup ()
{
	...
	pinMode(CONFIG_PIN_IRQ, INPUT);

	pinMode(CONFIG_PIN_SCK, OUTPUT);
	pinMode(CONFIG_PIN_MOSI, OUTPUT);
	pinMode(CONFIG_PIN_MISO, INPUT);
	pinMode(CONFIG_PIN_CSN, OUTPUT);
	pinMode(CONFIG_PIN_CE, OUTPUT);
	...
	for (i = 0; i <= 40; i++)
	{
		...
		digitalWrite(CONFIG_PIN_LED, (i & 1) ? HIGH : LOW);
		delay(100);
	}
	...
	((unsigned char *) &code_block)[0] = eeprom_read_byte (0);
	((unsigned char *) &code_block)[1] = eeprom_read_byte (1);
